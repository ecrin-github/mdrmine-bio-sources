 ...
 * @author
 */
public class CtgConverterCSV extends BaseConverter {
    //
     ...
     */
    public CtgConverterCSV(ItemWriter writer, Model model) {
    ...
    public void parseAndStoreTrial(String[] lineValues) throws Exception {
        ...
        if (!ConverterUtils.isBlankOrNull(trialID) && !trialID.equals("NCT01027572") &&
            ...
            /* Study start date */
            String startDateStr = CtgConverterCSV.normaliseDateString(this.getAndCleanValue(lineValues, "Start Date"));
            this.parseStartDate(study, startDateStr);

            /* Study end date */
            // Last primary outcome measure data collection date
            String primaryCompletionDateStr = CtgConverterCSV
                    .normaliseDateString(this.getAndCleanValue(lineValues, "Primary Completion Date"));
            String completionDateStr = CtgConverterCSV
    ...
    public void parseSponsor(Item study, String sponsor) throws Exception {
        if (!ConverterUtils.isBlankOrNull(sponsor)) {
            ...
            this.createAndStoreClassItem(study, "Organisation",
                    new String[][] { { "contribType", ConverterCVT.CONTRIB_TYPE_SPONSOR }, { "name", sponsor } });
        }
    }
    ...
    public void parseAge(Item study, String ageStr) {
        if (!ConverterUtils.isBlankOrNull(ageStr)) {
            ...
            for (String ageRange : ageRanges) {
                if (ageRange.equalsIgnoreCase(CtgConverterCSV.AGE_CHILD)) { // "CHILD" (0-17)
                    minAge = Integer.MIN_VALUE;
                    if (maxAge < 17) {
                        maxAge = 17;
                    }
                } else if (ageRange.equalsIgnoreCase(CtgConverterCSV.AGE_ADULT)) { // "ADULT" (18-64)
                    if (minAge > 18) {
                        minAge = 18;
                    }
                    if (maxAge < 64) {
                ...
                } else if (ageRange.equalsIgnoreCase(CtgConverterCSV.AGE_OLDER_ADULT)) { // "OLDER_ADULT" (65+)
                    if (minAge > 65) {
                        minAge = 65;
                    }
            ...
            if (splitLocations.length > 0) {
                for (String location : splitLocations) {
                    ...
                    if (splitLocation.length < 2) {
                        ...
                        facility = CtgConverterCSV.cleanLocationSubstring(splitLocation[0]);
                        cityName = CtgConverterCSV.cleanLocationSubstring(splitLocation[1]);
                        countryName = CtgConverterCSV.cleanCountryString(splitLocation[1], splitLocation[2]);
                        ...
                        countryName = CtgConverterCSV.cleanCountryString(splitLocation[2], splitLocation[3]);
                        ...
                        countryName = CtgConverterCSV.cleanCountryString(splitLocation[3], splitLocation[4]);
                        ...
                        countryName = CtgConverterCSV.cleanCountryString(splitLocation[4], splitLocation[5]);
            ...
            for (String doc : splitDocuments) {
                ...
                if (mDoc.matches()) {
                    ...
                    if (g1Lower.contains("study protocol")) {
                        if (!instances.containsKey(ConverterCVT.O_TYPE_PROT)) {
                            instances.put(ConverterCVT.O_TYPE_PROT, new ArrayList<String>());
                        }
                        instances.get(ConverterCVT.O_TYPE_PROT).add(g2);
                    }
                    if (g1Lower.contains("statistical analysis plan")) {
                        if (!instances.containsKey(ConverterCVT.O_TYPE_SAP)) {
                            instances.put(ConverterCVT.O_TYPE_SAP, new ArrayList<String>());
                        }
                        instances.get(ConverterCVT.O_TYPE_SAP).add(g2);
                    }
                } else {
